...
#2.启动mysql容器
docker run --name mysql --network exam -v mysql:/var/lib/mysql -e TZ=Asia/Shanghai -e MYSQL_ALLOW_EMPTY_PASSWORD=yes -d mysql:8.0.23
#3.进入容器并新建exam数据库
...
#5.启动项目容器
docker run --name jdk --network exam -v jar:/root -e TZ=Asia/Shanghai -d adoptopenjdk:11-jre-hotspot java -jar /root/exam-backend-0.0.1-SNAPSHOT.jar --spring.profiles.active=prod
